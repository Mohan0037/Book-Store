...
export default function Asignup() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    try {
      const response = await axios.post("http://localhost:8000/api/admin/signup", { name, email, password });
  ...
  return (
    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "100vh", padding: "1rem" }}>
      <div className="premium-card" style={{ padding: "2.5rem", width: "100%", maxWidth: "450px" }}>
        <h2 style={{ fontSize: "2rem", marginBottom: "0.5rem", textAlign: "center" }}>Create Admin Account</h2>
        <p style={{ color: "var(--text-secondary)", marginBottom: "2rem", textAlign: "center" }}>Register to manage BookStore operations</p>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
          ...
          <button type="submit" className="premium-btn" style={{ marginTop: "1rem" }}>Register</button>
        </form>

        <p style={{ marginTop: "1.5rem", textAlign: "center", color: "var(--text-secondary)", fontSize: "0.875rem" }}>
          Already have an account? <span onClick={() => navigate("/admin/login")} style={{ color: "var(--accent-primary)", cursor: "pointer", fontWeight: "600" }}>Log In</span>
        </p>
      </div>
    </div>
  );
